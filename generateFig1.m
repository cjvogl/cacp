% Script to generate Figure 1: comparing CACP method to CP method for the 
% unit circle in R^2
...
N = length(M);
if (exist('circle_results.mat','file') ~=2 )
    ...
    for j = 1:N
        [e2,eI,numnz,condn,X,Y,Ucp,Ucacp,Uex] = shiftp_equation(M(j), 1);
        CP_L2_error(j) = e2(1);
    ...
    save('circle_results.mat','CP_L2_error','CACP_L2_error','CP_LI_error',...
        'CACP_LI_error','CP_nnz','CACP_nnz','CP_condition','CACP_condition');
else
    load('circle_results.mat');
end
...
plot(M,CACP_L2_error./CP_L2_error,'-bo',...
     M(2:end),CACP_LI_error(2:end)./CP_LI_error(2:end),'-rs','linewidth',lw,'markersize',ms);
...
saveas(gcf,'normalized_error_circle','png');
...
ax_height = outerpos(4) - ti(2) - ti(4);
set(gca,'Position',[left bottom ax_width, ax_height]);
saveas(gcf,'num_nonzeros_circle','png');